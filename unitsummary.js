...
router.get('/', async function(req, res, next) {
    ...
    */
    ...
    for (let i = 0; i < users.length; i++)
    {
        ...
        let tscore, boolTSCore;

        if (cpa_result && cpa_result.total_score) {
            tscore = cpa_result.total_score;
            boolTSCore = tscore <= 25;
            cpa_result = JSON.parse(cpa_result.total_score);
        } else {
            tscore = 'N/A';
            boolTSCore = false;
